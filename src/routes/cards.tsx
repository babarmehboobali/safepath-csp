...
function Cards() {
  ...
  const latest = useMemo(() => {
    const map = new Map<string, { dueAt: number; rating: CardReviewRating; intervalDays: number }>();
    for (const review of session.cardReviews) {
      const current = map.get(review.cardId);
      if (!current || review.at > (current.dueAt - review.intervalDays * 86400000)) map.set(review.cardId, review);
    }
    return map;
  }, [session.cardReviews]);
  ...
  function reshuffle(nextMode = mode, nextKind = kindFilter) {
    const base = nextMode === "all" && nextKind === "all" ? source : shuffleCards(source);
    ...
    <Shell>
      <div className="sp-wrap max-w-5xl space-y-6">
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="sp-card p-4"><div className="text-xs text-fg-muted">Difficulty</div><div className="text-2xl font-semibold">L{card.difficulty}/5</div></div>
        </section>

        <div className="flex flex-wrap gap-2" aria-label="Review mode">
          {(["all", "due", "new", "weak"] as Mode[]).map((m) => <button key={m} type="button" className={mode === m ? "sp-btn sp-btn-primary" : "sp-btn sp-btn-ghost"} onClick={() => { setMode(m); reshuffle(m, kindFilter); }}>{m === "all" ? "All" : m === "due" ? "Due now" : m === "new" ? "New" : "Needs repair"}</button>)}
        </div>
        <div className="flex flex-wrap gap-2" aria-label="Card type filter">
          {(["all", "core", "apply", "contrast", "trap", "decision", "teachback", "formula", "rapid"] as const).map((kind) => <button key={kind} type="button" className={kindFilter === kind ? "sp-btn sp-btn-primary" : "sp-btn sp-btn-ghost"} onClick={() => { setKindFilter(kind); reshuffle(mode, kind); }}>{kind === "all" ? "All types" : KIND_LABELS[kind]}</button>)}
        </div>

        <article className="sp-card overflow-hidden">
          <div className="border-b p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="font-mono text-xs text-fg-subtle">Class {card.classId} · {card.domain}</div><h2 className="mt-1 text-lg font-semibold">{card.title}</h2></div><div className="flex gap-2 text-xs"><span className="rounded-full border px-3 py-1">{KIND_LABELS[card.kind]}</span><span className="rounded-full border px-3 py-1">Difficulty {card.difficulty}/5</span></div></div></div>
          <button type="button" className="min-h-[360px] w-full p-7 text-left" onClick={() => setFlip((v) => !v)} aria-label="Reveal or hide flashcard answer"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-fg-muted">{flip ? "Reasoning / answer" : "Active recall"}</div><p className="mt-7 max-w-4xl font-serif text-2xl leading-relaxed">{flip ? card.back : card.front}</p><p className="mt-8 text-sm text-accent">{flip ? "Tap to hide" : "Think first. Then reveal."}</p></button>
          <div className="border-t p-4">{!flip ? <p className="text-sm text-fg-muted">Retrieve first. Say the answer aloud if possible. Then reveal and compare your reasoning, not just the wording.</p> : <div><p className="mb-3 text-sm font-semibold">How well did you retrieve it?</p><div className="grid gap-2 sm:grid-cols-4">{RATINGS.map((r) => <button key={r.id} type="button" onClick={() => rate(r.id)} className={lastRating === r.id ? "sp-btn sp-btn-primary text-left" : "sp-btn sp-btn-ghost text-left"}><span className="block font-semibold">{r.label}</span><span className="block text-xs opacity-80">{r.hint}</span></button>)}</div></div>}</div>
        </article>
