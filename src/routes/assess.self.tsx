...
export const Route = createFileRoute("/assess/self")({ component: Self });

function Self() {
  const [items, setItems] = useState<BankItem[] | null>(null);
  useEffect(() => {
    const t = window.setTimeout(() => setItems(pickWeighted(100, readSession().track)), 30);
    return () => window.clearTimeout(t);
  }, []);
  if (!items?.length) {
    return (
      <Shell>
        <div className="sp-wrap"><p className="sp-kicker">Self-assessment</p><h1 className="sp-title mt-2 text-3xl">Building sitting…</h1></div>
      </Shell>
    );
  }
  return (
    <CbtSitting
      title="Self-assessment · 100 items"
      items={items}
      minutes={timerMinutesFor(100)}
