...
export function DisplaySettingsPanel() {
  ...
  const set = (key: keyof typeof display, value: never) => setDisplay(key as never, value);
  return (
    <>
      ...
      {open ? <div className="sp-settings-backdrop" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
        <aside className="sp-settings-panel" role="dialog" aria-modal="true" aria-label="Display and layout settings">
          ...
          <div className="sp-settings-scroll">
            ...
            <section><h3>Font size</h3><div className="sp-settings-segment">{([["small", "Small"], ["default", "Default"], ["large", "Large"], ["xlarge", "Extra large"]] as const).map(([id, label]) => <button key={id} type="button" className={display.fontScale === id ? "is-selected" : ""} onClick={() => set("fontScale", id as never)}>{label}</button>)}</div></section>
            <section><h3>Page density</h3><div className="sp-settings-segment">{([["compact", "Compact"], ["comfortable", "Comfortable"], ["spacious", "Spacious"]] as const).map(([id, label]) => <button key={id} type="button" className={display.density === id ? "is-selected" : ""} onClick={() => set("density", id as never)}>{label}</button>)}</div></section>
            <section><h3>Website arrangement</h3><p className="sp-settings-help">Choose where the study navigation appears. Hidden removes the desktop sidebar.</p><div className="sp-settings-segment">{([["left", "Left → Right"], ["right", "Right ← Left"], ["hidden", "No sidebar"]] as const).map(([id, label]) => <button key={id} type="button" className={display.sidebar === id ? "is-selected" : ""} onClick={() => set("sidebar", id as never)}>{label}</button>)}</div></section>
            <section><h3>Content width</h3><div className="sp-settings-segment">{([["standard", "Standard"], ["wide", "Wide"], ["full", "Full"]] as const).map(([id, label]) => <button key={id} type="button" className={display.contentWidth === id ? "is-selected" : ""} onClick={() => set("contentWidth", id as never)}>{label}</button>)}</div></section>
            <section><h3>Display options</h3><label className="sp-settings-toggle"><span><b>Animations</b><small>Transitions and hover movement</small></span><input type="checkbox" checked={display.showAnimations} onChange={(e) => set("showAnimations", e.target.checked as never)} /></label><label className="sp-settings-toggle"><span><b>Sidebar labels</b><small>Show text beside navigation icons</small></span><input type="checkbox" checked={display.showSidebarLabels} onChange={(e) => set("showSidebarLabels", e.target.checked as never)} /></label></section>
          </div>
