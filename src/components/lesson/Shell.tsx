...
const SIDEBAR = [
  { to: "/", label: "Study Home", icon: "⌂" }, { to: "/studio", label: "All Modules", icon: "▦" }, { to: "/studio", label: "Current Module", icon: "▣" }, { to: "/studio", label: "Study Notes", icon: "▤" }, { to: "/ready", label: "Key Points", icon: "✣" }, { to: "/practice", label: "Practice Questions", icon: "✓" }, { to: "/games", label: "Flashcards", icon: "▣" }, { to: "/mock", label: "Quizzes", icon: "◉" }, { to: "/bank", label: "Glossary", icon: "▥" }, { to: "/plan", label: "Mind Maps", icon: "⌘" }, { to: "/calculator", label: "Downloads", icon: "⇩" }, { to: "/account", label: "Bookmarks", icon: "♧" },
] as const;

export function Shell({ children }: { children: React.ReactNode }) {
  const [done, setDone] = useState(0); const [name, setName] = useState(""); const [open, setOpen] = useState(false);
  const { appearance } = useAppearance(); const location = useLocation();
