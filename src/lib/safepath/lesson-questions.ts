...
export type LessonQuestion = { id: string; classId: number; item: ClassItem };
...
export const getLessonQuestions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<LessonQuestion[]> => {
    ...
    const rows = await sql.query<{
      id: string; classId: number; stem: string; optionsJson: string; answerIndex: number; explanation: string;
      errorCode: string; taskCode: string; difficulty: string; distractorsJson: string | null; scenarioText: string | null;
      standardReference: string | null; difficultyLevel: string | null; optionTrapsJson: string | null; fieldTakeaway: string | null; mathJson: string | null;
    }>(
      `SELECT "id", "classId", "stem", "optionsJson", "answerIndex", "explanation", "errorCode", "taskCode", "difficulty", "distractorsJson", "scenarioText", "standardReference", "difficultyLevel", "optionTrapsJson", "fieldTakeaway", "mathJson"
       FROM "Question" WHERE "classId" = $1 ORDER BY "id"`,
      [classId],
    );
    return rows.map((row) => {
      ...
      const item: ClassItem = {
        stem: row.stem, options, answerIndex: Number(row.answerIndex), explanation: row.explanation,
        errorCode: normalizeErrorCode(row.errorCode), taskCode: row.taskCode, difficulty, distractors,
        scenarioText: row.scenarioText ?? undefined, standardReference: row.standardReference ?? undefined,
        difficultyLevel: normalizeDifficultyLevel(row.difficultyLevel, difficulty), optionTraps,
        fieldTakeaway: row.fieldTakeaway ?? undefined, stepByStepMath: toStepByStepMath(row.mathJson),
