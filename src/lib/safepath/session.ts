...
import type { ErrorType } from "./adaptive-engine";
...
export type LearningAttempt = { id: string; classId: number; domain: number; topic: string; competency?: string; difficulty: number; correct: boolean; responseTimeSeconds: number; confidence: number; errorType?: ErrorType; at: number };
export type CardReviewRating = "again" | "hard" | "good" | "easy";
export type CardReview = { id: string; cardId: string; classId: number; kind?: string; rating: CardReviewRating; responseTimeSeconds: number; at: number; dueAt: number; intervalDays: number };
export type LessonRecord = { slides: string[]; drillScore: number; drillTotal: number; at: number };
export type StudioSession = { email: string; name: string; agreed: boolean; completed: number[]; track: TrackId; industry: IndustryId; exams: ExamRecord[]; studyDates: string[]; knownCards: number[]; missed: number[]; lessons: Record<number, LessonRecord>; flaggedClasses: number[]; attempts: LearningAttempt[]; cardReviews: CardReview[] };
function empty(): StudioSession { return { email: "", name: "", agreed: false, completed: [], track: "recommended", industry: "general", exams: [], studyDates: [], knownCards: [], missed: [], lessons: {}, flaggedClasses: [], attempts: [], cardReviews: [] }; }
...
export function readSession(): StudioSession {
  ...
  try {
    ...
    return { email: String(parsed.email || ""), name: String(parsed.name || ""), agreed: Boolean(parsed.agreed), completed: Array.isArray(parsed.completed) ? parsed.completed.map(Number) : [], track: parsed.track === "compact" || parsed.track === "maximum" ? parsed.track : "recommended", industry: parsed.industry === "oil" || parsed.industry === "build" ? parsed.industry : "general", exams: Array.isArray(parsed.exams) ? parsed.exams.map((row) => ({ at: Number(row.at) || Date.now(), score: Number(row.score) || 0, total: Number(row.total) || 0, kind: row.kind, pct: row.pct })) : [], studyDates: Array.isArray(parsed.studyDates) ? parsed.studyDates.map(String) : [], knownCards: Array.isArray(parsed.knownCards) ? parsed.knownCards.map(Number) : [], missed: Array.isArray(parsed.missed) ? Array.from(new Set(parsed.missed.map(Number))).slice(0, 200) : [], lessons: parsed.lessons && typeof parsed.lessons === "object" ? parsed.lessons as Record<number, LessonRecord> : {}, flaggedClasses: Array.isArray(parsed.flaggedClasses) ? Array.from(new Set(parsed.flaggedClasses.map(Number).filter((id) => Number.isFinite(id) && id > 0))) : [], attempts: Array.isArray(parsed.attempts) ? parsed.attempts.map((row) => ({ id: String(row.id || `${row.classId}-${row.at || Date.now()}`), classId: Number(row.classId) || 0, domain: Number(row.domain) || 0, topic: String(row.topic || ""), competency: row.competency ? String(row.competency) : undefined, difficulty: Number(row.difficulty) || 3, correct: Boolean(row.correct), responseTimeSeconds: Number(row.responseTimeSeconds) || 0, confidence: Number(row.confidence) || 0, errorType: row.errorType, at: Number(row.at) || Date.now() })).slice(-1000) : [], cardReviews: Array.isArray(parsed.cardReviews) ? parsed.cardReviews.map((row) => ({ id: String(row.id || `${row.cardId}-${row.at || Date.now()}`), cardId: String(row.cardId || ""), classId: Number(row.classId) || 0, kind: row.kind ? String(row.kind) : undefined, rating: row.rating === "again" || row.rating === "hard" || row.rating === "easy" ? row.rating : "good", responseTimeSeconds: Math.max(0, Number(row.responseTimeSeconds) || 0), at: Number(row.at) || Date.now(), dueAt: Number(row.dueAt) || Date.now(), intervalDays: Math.max(0, Number(row.intervalDays) || 0) })).slice(-2000) : [] };
  } catch { return empty(); }
}

export function writeSession(next: StudioSession) {
  next.attempts = next.attempts.slice(-1000);
  next.cardReviews = next.cardReviews.slice(-2000);
...
export function recordMissed(classIds: number[]) { const s = markStudy(); const set = new Set(s.missed); classIds.forEach((id) => set.add(id)); s.missed = Array.from(set).slice(0, 200); writeSession(s); return s; }
export function recordAttempt(attempt: Omit<LearningAttempt, "id" | "at">) { const s = markStudy(); s.attempts.push({ ...attempt, id: `${attempt.classId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, at: Date.now() }); if (!attempt.correct && !s.missed.includes(attempt.classId)) s.missed = [attempt.classId, ...s.missed].slice(0, 200); writeSession(s); return s; }
...
export function recordCardReview(input: Omit<CardReview, "id" | "at" | "dueAt" | "intervalDays">) {
  const s = markStudy();
  const previous = [...s.cardReviews].reverse().find((r) => r.cardId === input.cardId);
  const base = previous?.intervalDays || 1;
  const multiplier = input.rating === "again" ? 0.25 : input.rating === "hard" ? 1.25 : input.rating === "easy" ? 4 : 2.5;
  const intervalDays = Math.max(input.rating === "again" ? 0.04 : 1, Math.min(120, base * multiplier));
  const review: CardReview = { ...input, id: `${input.cardId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, at: Date.now(), intervalDays, dueAt: Date.now() + intervalDays * 86400000 };
  s.cardReviews.push(review); writeSession(s); return review;
}
export function markCardKnownById(cardId: string, classId: number) { const s = readSession(); if (!s.knownCards.includes(classId)) s.knownCards.push(classId); writeSession(s); return s; }
