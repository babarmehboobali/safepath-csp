...
export function scoreCompetency(attempts: AdaptiveAttempt[], now = Date.now()): CompetencyState[] {
  ...
  return [...groups.values()].map((rows) => {
    ...
    const weaknessScore = clamp((weightedErrors / weightedTotal) * 0.65 + (1 - recentAccuracy) * 0.20 + slowPenalty * 0.10 + repeatPenalty * 0.05, 0, 1);
    const last = ordered.at(-1)!;
    let recommendedDifficulty = clamp(Math.round(last.difficulty), 1, 5);
    if (recentAccuracy >= 0.85 && slowPenalty < 0.25) recommendedDifficulty = clamp(recommendedDifficulty + 1, 1, 5);
...
export function buildNextActions(states: CompetencyState[], limit = 3) {
  return states.slice(0, limit).map((state) => {
    if (state.weaknessScore >= 0.65) return { type: "remediation", domain: state.domain, topic: state.topic, count: 10, difficulty: Math.max(1, state.recommendedDifficulty - 1) };
    if (state.weaknessScore >= 0.40) return { type: "targeted-practice", domain: state.domain, topic: state.topic, count: 10, difficulty: state.recommendedDifficulty };
    return { type: "mixed-scenario", domain: state.domain, topic: state.topic, count: 8, difficulty: state.recommendedDifficulty };
  });
}
